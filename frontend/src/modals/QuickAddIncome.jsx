...
import { getIncomePresets } from "../api/income";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faArrowTrendUp, faXmark } from "@fortawesome/free-solid-svg-icons";
import Loader from "../components/Loader";

function QuickAddIncome({onClose, onSelectIncomePreset}) {
  const [incomePresets, setIncomePresets] = useState([]);
    ...
    <div className="bg-[#0a0818] border border-[#2e2460] rounded-xl p-8 w-auto sm:w-170">
      ...
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl sm:text-3xl font-bold text-[#e2d9f3] syne-heading">
          <FontAwesomeIcon icon={faArrowTrendUp} className="me-3" /> Quick Add Income
        </h2>
      </div>
          ...
          <div>
            {incomePresets.length === 0 ? (
              <p className="text-[#6b5f8a] text-sm">
                No presets yet. Add more income records first.
              </p>
            ) : (
              <>
              <div className="grid sm:grid-cols-2 gap-5">
                {incomePresets.map((preset, index) => (
                  <button
                    key={index}
                    onClick={() => onSelectIncomePreset(preset)}
